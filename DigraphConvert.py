...
def digraphToBipartite(digIn):
    ...
    bipOut = nx.Graph()
    bipOut.add_nodes_from(range(1,n+1), bipartite=0) #first n are out nodes, second n are in nodes
    bipOut.add_nodes_from(range(n+1,2*n+1), bipartite=1) #outs have bipartite flag 0, ins have bipartite flag 1
    diEdges = digIn.edges()
    for j in diEdges:
...
def CelegansLabelConvert(G):
    ...
    nodeList = G.nodes()
    labelDict = dict(zip(range(len(nodeList)), nodeList))
